...
server.post("/chores/", function (req, res, next) {
  const error = validateChore(req.body);
  if (error) {
...
function validateChore(chore) {
  if (!chore.name) return "Chore is required.";
  if (!chore.userId) return "UserId is required.";
